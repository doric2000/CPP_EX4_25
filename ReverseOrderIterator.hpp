 ...
 * 
 * ReverseOrderIterator takes a copy of the container’s data, sorts it in
 * Reverse order, and then allows sequential access via iterator semantics.
 */
template<typename T>
class ReverseOrderIterator {
private:
    /// Sorted copy of all elements, in Reverse order
...
public:
    /**
     * @brief Construct a new ReverseOrderIterator.
     * 
     * Builds a sorted copy of all_data (Reverse), and sets the starting index.
     * 
     ...
     * 
     * @return ReverseOrderIterator&  Reference to this iterator after increment.
     * @throws std::out_of_range if index >= sorted_data.size()
     */
     ...
     * @return ReverseOrderIterator  Copy of this iterator before increment.
     * @throws std::out_of_range if index >= sorted_data.size()
     */
    ReverseOrderIterator operator++(int) {
        ReverseOrderIterator copy = *this;
     ...
     * 
     * @param other  Another ReverseOrderIterator to compare with.
